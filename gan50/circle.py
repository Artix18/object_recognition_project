...
def get_distribution_sampler(mu, sigma):
    def truc():
        ...
        #ipdb.set_trace()
        return torch.Tensor([np.cos(theta[0]), np.sin(theta[0])])
...
D = Discriminator(input_size=g_output_size, hidden_size=d_hidden_size, output_size=d_output_size)
...
for epoch in range(num_epochs):
    ...
    if epoch % print_interval == 0:
        print("%s: D: %s/%s G: %s " % (epoch,
                                                            ...
                                                            extract(g_error)[0]))
        plt.scatter(d_fake_data[:,0].data.numpy(), d_fake_data[:,1].data.numpy())
        plt.ion()
        plt.show()
        plt.pause(.001)
    if False and epoch % print_interval == 0:
...
for i in range(200):
    ...
    #ipdb.set_trace()
    xp = np.append(xp, d_fake_data[0,0].data.numpy())
    yp = np.append(yp, d_fake_data[0,1].data.numpy())
